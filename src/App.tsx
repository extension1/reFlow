...
const PRESETS: Record<string, {
  ...
  torus_plasma: {
    ...
    smokeMode: 'streamers',
    is2D: false,
    torusMajorRadius: 3.2,
    torusMinorRadius: 1.0
  },
  torus_thin_ring: {
    name: 'Thin Ring Vortex',
    description: 'Narrow torus with high R/r ratio — generates distinctive ring vortex shedding patterns.',
    activeObject: 'Custom',
    flowSpeed: 2.4,
    viscosity: 2.5,
    angleofAttack: 10,
...
export default function App() {
  ...
  // --- Torus & Custom Model Loading & Panel Toggles ---
  const [torusMajorRadius, setTorusMajorRadius] = useState(3.2);
  const [torusMinorRadius, setTorusMinorRadius] = useState(1.0);
  const [leftPanelOpen, setLeftPanelOpen] = useState(true);
  const [bottomPanelOpen, setBottomPanelOpen] = useState(true);
  const [loadedModelBuffer, setLoadedModelBuffer] = useState<ArrayBuffer | null>(null);
  const [loadedModelName, setLoadedModelName] = useState<string | null>(null);
  const [loadedModelSize, setLoadedModelSize] = useState<number>(0);
  ...
  const handleGLBUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      setLoadedModelBuffer(reader.result as ArrayBuffer);
      setLoadedModelName(file.name);
      setLoadedModelSize(file.size);
      setActiveObject('GLBModel');
    };
  ...
  const handleRemoveModel = () => {
    ...
    setLoadedModelSize(0);
    if (activeObject === 'GLBModel') {
      setActiveObject('Wing');
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (gl) {
        ...
        if (debugInfo) {
          gpuString = gl.getParameter((debugInfo as any).UNMASKED_RENDERER_VENDOR_STRING) + ' ' + gl.getParameter((debugInfo as any).UNMASKED_RENDERED_GPU_STRING);
        } else {
  ...
  const loadPreset = (key: string) => {
    ...
    // Set torus params if present in preset
    if (p.torusMajorRadius !== undefined) {
      setTorusMajorRadius(p.torusMajorRadius);
    }
    if (p.torusMinorRadius !== undefined) {
      setTorusMinorRadius(p.torusMinorRadius);
    }
...
`;
  ...
  // Format file size for GLB display
  ...
  return (
    <div className="flex flex-col w-screen h-screen bg-[#060608] text-[#e2e8f0] overflow-hidden font-mono selection:bg-[#7DF9FF]/30 selection:text-white relative">
      ...
      <div className="flex flex-1 w-full overflow-hidden relative">
        {/* Left Control Sidebar — Collapsible */}
        <aside
          className="border-r border-[#1b1b22] bg-[#09090c] flex flex-col overflow-hidden z-10 shrink-0 select-none transition-all duration-300 ease-in-out"
          style={{ width: leftPanelOpen ? '18rem' : '2.5rem' }}
        >
          {/* Panel toggle button */}
          <div className={`flex items-center ${leftPanelOpen ? 'justify-between' : 'justify-center'} px-2 py-2 border-b border-[#1b1b22] shrink-0`}>
            {leftPanelOpen && (
              <span className="text-[9px] font-bold uppercase tracking-widest text-gray-500 pl-2">Controls</span>
            )}
            <button
              onClick={() => setLeftPanelOpen(prev => !prev)}
              className="flex items-center justify-center w-6 h-6 rounded border border-[#7DF9FF]/20 bg-[#7DF9FF]/5 text-[#7DF9FF] hover:bg-[#7DF9FF]/15 hover:border-[#7DF9FF]/40 transition-all duration-200 active:scale-90"
              title={leftPanelOpen ? 'Collapse controls panel' : 'Expand controls panel'}
            >
          ...
          {leftPanelOpen && (
            <div className="flex-1 overflow-y-auto">
              ...
              {(activeObject === 'GLBModel') && (
                <div className="p-4 border-b border-[#1b1b22] space-y-3">
                  <div className="flex items-center gap-2">
                    <Upload className="w-4 h-4 text-[#7DF9FF]" />
                    <span className="text-[10px] font-bold uppercase tracking-widest text-[#7DF9FF]">GLB Model Loader</span>
                  </div>

                  {loadedModelBuffer ? (
                    <div className="space-y-2.5">
                      {/* Loaded file info */}
                      <div className="flex items-center gap-2.5 p-2.5 border border-emerald-500/20 bg-emerald-500/5 rounded">
                        <div className="w-8 h-8 rounded border border-emerald-500/30 bg-emerald-500/10 flex items-center justify-center text-emerald-400 text-lg shrink-0">
                          📦
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] font-bold text-white truncate">{loadedModelName}</div>
                          <div className="text-[9px] text-emerald-400">{formatFileSize(loadedModelSize)}</div>
                        </div>
                      </div>
                      {/* Remove model button */}
                      <button
                        onClick={handleRemoveModel}
                        className="w-full py-1.5 px-3 border border-[#ff003c]/25 bg-[#ff003c]/5 hover:bg-[#ff003c]/15 text-[#ff003c] rounded text-[9px] font-bold uppercase transition-colors flex items-center justify-center gap-1.5"
                      >
                    ...
                    <div className="space-y-2">
                      {/* Upload prompt */}
                      <label className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-[#1b1b22] hover:border-[#7DF9FF]/40 bg-[#0b0b0f] rounded cursor-pointer transition-colors group">
                        <Upload className="w-6 h-6 text-gray-600 group-hover:text-[#7DF9FF] transition-colors mb-2" />
                        <span className="text-[10px] text-gray-500 group-hover:text-gray-300 font-bold uppercase tracking-wide">
                          Drop or click to upload
                        </span>
                        <span className="text-[8px] text-gray-600 mt-1">.glb / .gltf files accepted</span>
                        <input
                          ref={glbInputRef}
                          type="file"
                          accept=".glb,.gltf"
              ...
              {/* Torus Parameters Section */}
              {activeObject === 'Custom' && (
                <div className="p-4 border-b border-[#1b1b22] space-y-3">
                  <div className="flex items-center gap-2">
                    <Settings className="w-4 h-4 text-[#7DF9FF]" />
                    <span className="text-[10px] font-bold uppercase tracking-widest text-[#7DF9FF]">Torus Parameters</span>
                  </div>

                  {/* Major Radius */}
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[10px] uppercase font-bold text-gray-400">
                      <span>Major Radius (R)</span>
                      <span className="text-[#7DF9FF]">{torusMajorRadius.toFixed(1)}</span>
                    </div>
                    <input
                      type="range"
                      min="1.5"
                      ...
                      onChange={(e) => setTorusMajorRadius(parseFloat(e.target.value))}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between text-[10px] uppercase font-bold text-gray-400">
                      <span>Minor Radius (r)</span>
                      <span className="text-purple-400">{torusMinorRadius.toFixed(1)}</span>
                    </div>
                    <input
                      type="range"
                      min="0.3"
                      ...
                      onChange={(e) => setTorusMinorRadius(parseFloat(e.target.value))}
                      className="w-full accent-purple-400 bg-[#13131a] rounded-lg cursor-pointer h-1.5"
                    />
                  </div>

                  {/* R/r Ratio readout */}
                  <div className="flex items-center justify-between p-2 border border-[#1b1b22] bg-[#0d0d12] rounded text-[10px]">
                    <span className="text-gray-500 uppercase font-bold">Ratio R/r</span>
                    <span className="text-[#7DF9FF] font-extrabold text-xs">
                      {(torusMajorRadius / torusMinorRadius).toFixed(2)}
                    </span>
                  </div>

                  <div className="text-[8px] text-gray-500 italic leading-relaxed">
                    High R/r → thin ring (vortex shedding). Low R/r → fat donut (bluff body drag).
                  </div>
                </div>
              )}
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <main className="flex-1 min-h-[350px] relative overflow-hidden bg-[#0b0b0f] border-b border-[#1b1b22]">
            <WindTunnel
              ...
              loadedModelBuffer={loadedModelBuffer}
            />
          ...
          {/* Bottom Diagnostics Grid (Chart & AI Co-pilot) — Collapsible */}
          <div
            className="border-t border-[#1b1b22] bg-[#08080b] flex flex-col shrink-0 overflow-hidden transition-all duration-300 ease-in-out relative"
            style={{ height: bottomPanelOpen ? '16rem' : '2rem' }}
          >
            {/* Bottom panel toggle button — always visible */}
            <button
              onClick={() => setBottomPanelOpen(prev => !prev)}
              className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-0 z-20 flex items-center justify-center w-8 h-5 rounded-b border border-t-0 border-[#7DF9FF]/20 bg-[#09090c] text-[#7DF9FF] hover:bg-[#7DF9FF]/10 hover:border-[#7DF9FF]/40 transition-all duration-200 active:scale-90"
              title={bottomPanelOpen ? 'Collapse analysis panel' : 'Expand analysis panel'}
            >
            ...
            {bottomPanelOpen ? (
              <div className="flex-1 flex flex-col lg:flex-row overflow-hidden pt-1">
              ...
              <div className="flex items-center justify-center h-full text-[8px] text-gray-500 uppercase tracking-widest font-bold select-none pt-1">
                Chart & AI Consultant — Click ▲ to expand
              </div>
            )}
          </div>
        </div>
